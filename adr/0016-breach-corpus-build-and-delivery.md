...
**5. A missing, unreadable, or implausibly small corpus stops the application from starting.** Not a warning, not a disabled check. ADR-0012's whole argument for the offline filter is that it *removes* the fail-open/fail-closed question; a service that boots without its corpus reintroduces fail-open through the back door, and does it invisibly — every registration succeeds, the control simply is not there.

*Implausibly small* is the third case and it was added after review. The builder supports a range limit for smoke tests, and a range-limited run produces a file that is valid in every observable respect — right magic, right format, plausible digest — while covering a fraction of the 1,048,576 prefix ranges and therefore waving through nearly every breached password. The publishing workflow refuses to release one, but **a guard that only lives in CI protects CI**. So the service also refuses a corpus holding fewer than a million digests: an order of magnitude below the ~10.5M a real one holds and three above what a smoke build produces. It catches a full run that went wrong, too — a parser change that silently dropped most entries lands here, where nothing inspecting the build inputs could see it.
