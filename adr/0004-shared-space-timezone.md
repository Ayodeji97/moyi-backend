# ADR-0004 — One anchor timezone per Space
...
## Decision
Each Space chooses one IANA anchor timezone at creation. All Space-day boundaries, reveal timing and streak evaluation use it. Members' *notification* times remain in their own local timezones.

## Consequences
**Positive:** one shared day, one shared streak, one reveal — which is exactly what "our day together" means and can be explained in a sentence. Vastly simpler to implement correctly and to reason about in support. Only one clock to test against per Space.
...
## Mitigations
Onboarding explains the choice clearly and defaults to the creator's device zone. The compose screen shows which Space-day an entry will be filed under whenever it differs from the writer's local date.

## Revisit when
Beta feedback shows the anchor is a genuine source of friction, or the ratio of solo days is materially higher for Spaces with a large timezone separation — which is measurable from `gratitude_days_solo_total` segmented by member offset.
