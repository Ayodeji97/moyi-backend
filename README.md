...
**The API contract.** `contracts/openapi.json` is the OpenAPI document the
client is generated from (doc 06 §5, ADR-0024). It is generated by springdoc
from the running application and committed; `OpenApiContractTest` in `app`
validates it and fails when the committed file is stale — the message tells
you the one `cp` to run, and the diff you then see in the PR *is* the API
changing. Locally the live document is at `/v3/api-docs` under the `local`
profile, behind a bearer like everything else; production serves none. On a
pull request, `.github/workflows/api-contract.yml` diffs the file against
`main` with oasdiff and fails on a breaking change unless the PR carries the
`breaking-api-change` label; on merge it attaches the file to the rolling
`api-latest` release.
